...
public class LearningServiceTest {
    ...
    public void shouldSendLineManagerNotificationForCompletedLearning() {
        ...
        String username2 = "user@example.com";
        ...
        identityDTO.setUsername(username2);
        ...
        Instant completedInstant = completedDate.toInstant(ZoneOffset.UTC);
        ...
        CompletedLearning completedLearning = new CompletedLearning(courseRecord, completedInstant);
        List<CompletedLearning> completedLearningList = Arrays.asList(completedLearning);
        ...
        when(completedLearningService.findAll()).thenReturn(completedLearningList);
        when(scheduledNotificationsService.shoudSendNotification(uid1, id1, completedDate)).thenReturn(true);

        learningService.sendLineManagerNotificationForCompletedLearning();

        verify(scheduledNotificationsService).sendNotification(username, name, uid1, courseRecord);
        verify(completedLearningService).delete(completedLearning);
    }
}